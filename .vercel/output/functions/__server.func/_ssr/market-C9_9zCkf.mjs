...
import { i as getSql, t as authMiddleware } from "./middleware-D5FSTWHz.mjs";
import { cn as _enum, gn as object, hn as number, yn as string } from "../_libs/@better-auth/core+[...].mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/market-C9_9zCkf.js
...
var listVehicles = createServerFn({ method: "GET" }).validator(object({
	...
	if (q) list = list.filter((v) => v.title.toLowerCase().includes(q) || v.brand.toLowerCase().includes(q) || v.model.toLowerCase().includes(q) || v.city.toLowerCase().includes(q));
	if (data.brand) list = list.filter((v) => v.brand === data.brand);
	if (data.listingType) list = list.filter((v) => v.listingType === data.listingType || v.listingType === "ambos");
